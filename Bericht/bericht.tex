...
Dafür wurde zuerst die Relevanz von Oberflächenfahrzeugen in Katastrophenfällen kurz betrachtet.
Die notwendigen Funktionalitäten wurden in Python und C++ unter Nutzung von ROS und des von ROS bereitgestellten Navigation Stacks implementiert. Um die Funktionsweise testen zu können, wurde außerdem eine Simulation des Bootes auf Basis eines bestehenden Simulators erstellt.\\
Die Funktionsweise des Bootes wurde im Simulator in mehreren Szenarien getestet. Dabei konnte festgestellt werden, dass sich das Boot korrekt verhält. 
\end{abstract}
...
Die Simulationsumgebung stellt vier verschiedene Boot-Modelle bereit: ein Airboot, ein Differentialboot, ein Ruderboot und ein Segelboot \cite{paravisi2019}. Wir haben das Differentialboot-Modell als Grundlage für unser Modell genommen, da es sich bei unserem USV ebenfalls um ein Boot mit zwei Motoren am Heck (s. Abbildung \ref{boot}) handelt. Wir haben zwei Dateien identifiziert, die das Boot modellieren: \texttt{diffboat.xacro} (modelliert unter anderem die Motoren) und \texttt{boat\_subdivided\_validated.xacro} (modelliert unter anderem Sensoren). Diese Dateien wurden als Grundlage genutzt, um unser Boot zu modellieren.
...
Darüber hinaus verfügt das Boot noch über zwei Motoren für die Fortbewegung und eine 4G/WIFI-Anbindung zur Kommunikation (s. Abbildung \ref{boot}).\\
...
Für den Map Server haben wir eine schwarz-weiß Karte der in der Simulation dargestellten Region (\ref{diluvio}) erstellt. Dabei stellen weiße Regionen befahrbare und schwarze unbefahrbare Gebiete dar. Auf Basis der Karte wird vom Map Server ein Occupancy Grid erstellt, das letztendlich vom globalen Planer genutzt wird, um einen befahrbaren Pfad zu generieren:

\lstinputlisting[label={map-param}, caption={Parameter der Map}, language=yaml, breaklines=true]{../maps/map_2_orig.yaml}
...
Als globale Planer wird der \texttt{GlobalPlanner} aus dem Paket \texttt{global\_planner} \footnote{\url{http://wiki.ros.org/global_planner}} mit Standardeinstellungen verwendet.
...
Die in \texttt{recovery\_behaviors} gelisteten Verhalten werden in \ref{recovery} genauer erläutert. Durch \texttt{max\_planning\_retries} wird bestimmt, dass nach zehn fehlgeschlagenen Versuchen die Verhalten der Reihe nach ausgeführt werden.


\subsection{Vermeidung unerwarteter Hindernisse}

Die Vermeidung von Hindernissen wird mit Hilfe des vorderen Ultraschallsensors und des in \ref{sensor-topics} erwähnten Plugins vom Navigation Stack erreicht. Um auch große Hindernisse in der Pfadplanung berücksichtigen zu können, werden Hindernisse in die globale Costmap eingetragen und sich damit gemerkt.
...
Auf Gewässern kann es viele (auch nicht dynamische) Hindernisse geben, die auf Karten nicht vermerkt sind, zum Beispiel ankernde Boote, Stege, oder Bojen. Diesen Hindernissen muss nicht nur ausgewichen werden, es ist auch notwendig, einen neuen Zielpunkt zu setzen, sollten solche Hindernisse einen Zielpunkt über einen längeren Zeitraum blockieren. Dafür werden mehrere Recovery-Verhalten eingesetzt, wobei zwei davon neu implementiert wurden. Da die Verhalten über \texttt{pluginlib} \footnote{\url{http://wiki.ros.org/pluginlib}} eingebunden werden, muss für die Implementierung C++ verwendet werden.
Falls der Planer merkt, dass der aktuelle Plan nicht durchführbar ist, wird als erstes das \texttt{BackupRecovery} durchgeführt. Dabei wird langsam für drei Sekunden zurückgesetzt und zufällig in eine Richtung gedreht, mit dem Ziel danach einen besseren Plan finden zu können. Falls das scheitert wird als nächstes die lokale Costmap durch \texttt{ClearCostmapRecovery} aus dem Paket \texttt{clear\_costmap\_recovery} \footnote{\url{wiki.ros.org/clear_costmap_recovery}} geleert, um auf eventuell falsch erkannte Hindernisse zu reagieren. Falls weiterhin kein fahrbarer Pfad existiert, wird \texttt{SkipGoalRecovery} durchgeführt und das aktuelle Ziel übersprungen.

\subsection{Homing Verhalten}

Da das Boot nicht auf dem Wasser verloren gehen oder unter gehen soll, haben wir das \texttt{measurements\_pub} Skript um einen Subscriber ergänzt, der auf das Safety-Topic subscribt und dem Boot bestimmte Zielkoordinaten (s. Listing \ref{params}) gibt, falls der Batteriestand niedrig ist, oder sich Wasser im Boot befindet, sowie für den Fall, dass der komplette angegebene Bereich erfolgreich abgefahren wurde.

\section{Ergebnisse} \label{ergebnisse}

Ein Ergebnis unseres Projektes ist ein Simulator, der ein USV simuliert, das Daten mit Hilfe eines GPS-Gerätes, zweiter Ultraschallsensoren und einer IMU aufnimmt und die Daten auf dieselben Topics publisht, die auch das Boot nutzt, das uns zur Verfügung gestellt wurde.

% (s. Abbildung \ref{sonar}).
...
\subsection{Versuch Nummer 2: unerwartete Hindernisse} \label{v2}
...
\subsection{Versuch Nummer 3: Fläche mit unerreichbaren Regionen (erwartet)} \label{v3}
...
\subsection{Versuch Nummer 4: Fläche mit unerreichbaren Regionen (unerwartet)} \label{v4}
...
Jedoch hat lange gedauert, das gesamte Hindernis zu erfassen. Deshalb wurde ein weiterer Versuch (siehe \ref{versuch8}) unter gleichen Bedingungen, aber mit zusätzlichen Ultraschallsensoren durchgeführt.
...
\subsection{Versuch Nummer 5, 6: Batteriestand gering, Wasser im Boot} \label{v56}
...
\subsection{Versuch Nummer 7: Abbruch und Neustart der Messung} \label{v7}

In Versuchen Nummer 7 sollte eine Fläche von $9mx15m$ abgefahren und dabei in Abständen von $2m$ Messwerte aufgenommen werden. Nach elf Messpunkten wurde die Messung einmal abgebrochen und dann neu gestartet. Wie in Abbildung \ref{Versuch7} zu sehen, wurden dann erst wieder Messwerte ab Punkt zwölf aufgenommen. Da die Höhe und Breite des Bereichs nicht ganzzahlig durch die  Messabstände teilbar waren, wurden Bereiche an den Rändern ausgelassen.
...
\subsection{Versuch Nummer 8} \label{versuch8}

Der Versuchsaufbau ist gleich mit dem von Versuch Nummer 4, aber das Boot ist mit einem Array von Ultraschallsensoren ausgestattet, die in unterschiedlichen Winkeln nach Vorne und zur Seite des Bootes ausgerichtet sind. Dadurch konnte das große Hindernis schneller erfasst und umfahren werden.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{sonar.png}
	\caption{Boot mit mehreren Ultraschallsensoren}
	\label{sonar}
\end{figure}
...
Wie unsere Tests in Kapitel \ref{ergebnisse} zeigen, wurde unser am Anfang gestecktes Ziel, das USV durch Software zu ergänzen, die ein autonomes Befahren eines Gewässers ermöglicht, im Laufe der Projektarbeit erreicht, da das Boot sowohl autonom einen bestimmten Bereich abdecken, sowie seinen Fahrtverlauf an Hindernisse anpassen kann (zu sehen in den Versuchen in \ref{v2}, \ref{v3}, \ref{v4}). Auch die Implementation unseres Use Cases wurde erfolgreich durchgeführt, wie in den Abbildungen an den Messwerten zu erkennen. Raum zur Verbesserung sehen wir vor allem bei der Ausstattung mit Sensoren, da damit Hindernisse schneller umfahren werden könnten, sowie bei der Vermeidung dynamischer Hindernisse, da sich diese dauerhaft gemerkt werden, was bei einer großen Anzahl problematisch werden könnte.\\
Die Simulation des Bootes ist ebenfalls funktionsfähig. Eine Ausnahme bildet die Messung der Wassertiefe, welche bislang nicht korrekt gemessen werden kann, da der Ultraschallsensor die Wasseroberfläche als Hindernis wahrnimmt. Dies hat aber bislang keine Auswirkungen auf die Funktion des Bootes. Außerdem verfügt das Boot noch über weitere Sensoren, die wir im Modell nicht beachtet haben, da wir sie für unseren Anwendungsfall nicht benötigten. Diese sollten aber, analog zu den schon hinzugefügten Sensoren, durch Plugins relativ unproblematisch zu unserem Modell hinzugefügt werden können, so dass unser Modell auch für weitere Projekte eine gute Grundlage bildet.\\
Die Menge der Anwendungsfälle, in denen das Boot genutzt werden kann, ist bisher begrenzt. Die Messung der Wassertiefe könnte dabei helfen, vermisste Personen oder Gegenstände im Wasser zu lokalisieren. Andere Problemstellungen, wie zum Beispiel das Erkennen von Deichbrüchen und Verschmutzungen, können bis jetzt nicht abgedeckt werden. Aber auch für diese noch zu implementierenden Anwendungsfälle liefert das bereits erreichte autonome Befahren des Gewässers eine gute Grundlage.
